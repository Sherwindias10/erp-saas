...
const SaaSERPPlatform = () => {
  ...
  const [userRole, setUserRole] = useState(null); // 'superadmin', 'admin', 'manager', 'staff'
  ...
  // Tenant-specific data (simulating database per tenant)
  const [tenantData, setTenantData] = useState({
    1: { // Acme Manufacturing data
      customers: [
    ...
    2: { // TechStart Solutions data
      customers: [],
      products: [],
      salesOrders: [],
      ledgerEntries: []
    },
    3: { // Global Retail data
      customers: [],
      products: [],
      salesOrders: [],
      ledgerEntries: []
  ...
  // UI State
  const [activeView, setActiveView] = useState('login'); // 'login', 'superadmin', 'erp'
  ...
  const handleLogin = (email, password, role) => {
    ...
    if (role === 'superadmin') {
      setCurrentUser({ email: 'superadmin@yourcompany.com', name: 'Super Admin' });
  ...
  const addProduct = (product) => {
    const data = getCurrentTenantData();
    updateTenantData('products', [...data.products, { ...product, id: data.products.length + 1, stock: parseInt(product.stock || 0) }]);
    closeModal();
  };
  ...
  const addSalesOrder = (so, items) => {
    ...
    const newLedgerEntry = {
      ...
      debit: 0,
      credit: total,
      balance: (data.ledgerEntries[data.ledgerEntries.length - 1]?.balance || 0) + total
    };
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-600 to-purple-700 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={() => handleLogin(loginEmail, loginPassword, isSuperAdmin ? 'superadmin' : 'admin')}
              ...
              <tbody className="divide-y divide-gray-200">
                {tenants.map(tenant => (
                  <tr key={tenant.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">{tenant.createdDate}</td>
          ...
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <div>
              ...
              <p className="text-blue-100 text-sm">
                ...
                {currentTenant?.status === 'Trial' && ` • Trial ends: ${currentTenant?.trialEnds}`}
              </p>
            </div>
                      ...
                      <div className="flex">
                        ...
                        <div className="ml-3">
                          ...
                          <p className="mt-1 text-sm text-yellow-700">
                            Your trial ends on {currentTenant?.trialEnds}. Upgrade to continue using all features.
                          </p>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {data.ledgerEntries.length === 0 ? (
                          <tr>
                            <td colSpan="5" className="px-6 py-8 text-center text-gray-500">
                              No ledger entries yet. Create sales orders to see transactions.
                            </td>
                          </tr>
                        ) : (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {modalType === 'product' && (
                <>
                  ...
                  <input name="cost" type="number" placeholder="Cost Price" required className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
                  <input name="price" type="number" placeholder="Selling Price" required className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
                </>
              )}
  ...
  return (
    <>
      ...
      {activeView === 'erp' && renderCustomerERP()}
      <Modal />
    </>
  );
};
